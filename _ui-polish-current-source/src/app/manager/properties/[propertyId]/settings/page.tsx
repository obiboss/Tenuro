...
function formatFee(params: {
  ...
  return params.management_fee_type === "percentage"
...
export default async function ManagerPropertySettingsPage({
  ...
  const [
    ...
    listManagerPropertyServiceChargeSettings(supabase, {
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <section className="rounded-card border border-border-soft bg-white p-4 shadow-sm">
        <div>
          ...
          <p className="mt-1 text-sm font-semibold leading-6 text-text-muted">
            These settings control how rent is collected and divided.
          </p>
        </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <SummaryItem
            ...
            value={getCollectionLabel(property.collection_mode)}
          />
          <SummaryItem
      ...
      <ManagerPropertyTenantRequirements
        propertyId={property.id}
        landlordClientId={property.landlord_client_id}
        initialRequirements={tenantRequirements}
      />
      ...
      <section className="rounded-card border border-border-soft bg-white shadow-sm">
        <div className="border-b border-border-soft p-4">
          ...
          <p className="mt-1 text-sm font-semibold leading-6 text-text-muted">
            These older free-text clauses are included in tenancy agreements.
            They do not screen tenant applications.
          </p>
        </div>
        ...
        {agreementClauses.length > 0 ? (
          <div className="divide-y divide-border-soft">
            {agreementClauses.map((clause) => (
              <article key={clause.id} className="p-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <p className="font-black text-text-strong">
                    {clause.title}
                  </p>
                  <span className="w-fit rounded-full bg-surface px-3 py-1 text-xs font-black uppercase tracking-wide text-text-muted">
                    Agreement clause
                  </span>
                </div>
          ...
          <div className="p-4">
            <p className="rounded-card bg-surface p-4 text-sm font-semibold leading-6 text-text-muted">
              No older agreement clauses have been added.
            </p>
          </div>
        )}
      </section>
