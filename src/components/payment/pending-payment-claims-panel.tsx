    ...
    return (
      <div className="rounded-card border border-border-soft bg-white p-5">
        ...
        <p className="mt-2 text-sm font-semibold leading-6 text-text-muted">
          Tenant proof submissions and caretaker-reported payments will appear
      ...
      {claims.map((claim) => (
        ...
        >
          <div className="flex items-start justify-between gap-3">
            ...
            <Badge tone="warning">{claim.sourceLabel}</Badge>
          </div>

          <div className="mt-4 grid gap-2 text-sm font-bold text-text-muted sm:grid-cols-2">
            <p>Amount: {formatNaira(claim.amountPaid)}</p>
            <p>Paid: {claim.paymentDate}</p>
