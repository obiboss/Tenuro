  ...
  return (
    <div className="mx-auto max-w-6xl space-y-3">
      ...
      <section aria-labelledby="manager-attention-title" className="rounded-lg border border-border-soft bg-white">
        ...
        {visibleAttentionItems.length ? <ul className="divide-y divide-border-soft">{visibleAttentionItems.map((item) => { const classes = getToneClasses(item.tone); if (item.category === "rent") { const whatsappHref = buildWaMeUrl({ phoneNumber: item.phoneNumber ?? null, message: item.whatsappMessage ?? "" }); const phoneHref = item.phoneNumber?.trim() ? `tel:${item.phoneNumber.trim()}` : undefined; return <li key={item.id} className="flex flex-col gap-2.5 px-4 py-2.5 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className={cn("text-sm font-extrabold", classes.text)}>{item.title}</p><p className="mt-0.5 text-sm font-semibold text-text-normal">{item.subject}</p><p className="mt-0.5 text-xs font-semibold text-text-muted">{item.detail}</p></div><div className="flex shrink-0 items-center justify-end gap-2"><a aria-label="Message tenant on WhatsApp" href={whatsappHref} target="_blank" rel="noopener noreferrer" className={cn("inline-flex size-9 items-center justify-center rounded-md border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2", classes.border, classes.background, classes.text)}><MessageCircle className="size-4" aria-hidden="true" /></a>{phoneHref ? <a aria-label="Call tenant" href={phoneHref} className={cn("inline-flex size-9 items-center justify-center rounded-md border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2", classes.border, classes.background, classes.text)}><Phone className="size-4" aria-hidden="true" /></a> : null}</div></li>; } return <li key={item.id} className="flex flex-col gap-2.5 px-4 py-2.5 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className={cn("text-sm font-extrabold", classes.text)}>{item.title}</p><p className="mt-0.5 text-sm font-semibold text-text-normal">{item.subject}</p><p className="mt-0.5 text-xs font-semibold text-text-muted">{item.detail}</p></div><Link href={item.action.href} prefetch={false} className={cn("inline-flex min-h-10 shrink-0 items-center justify-center gap-2 rounded-md border px-3 text-sm font-extrabold", classes.border, classes.background, classes.text)}>{item.action.label}<ArrowRight className="size-4" aria-hidden="true" /></Link></li>; })}</ul> : <div className="px-4 py-2.5"><p className="text-sm font-semibold text-text-muted">Nothing needs your attention right now.</p></div>}
      </section>
