...
export async function POST(
  ...
  if (body?.action !== "whatsapp_shared" || typeof body.token !== "string") {
    return NextResponse.json({ message: "Invalid activity request." }, { status: 400 });
  }

  if (tool === "receipt") {
    await recordPublicReceiptWhatsappShare({ receiptId: documentId, token: body.token });
  } else if (tool === "agreement") {
    await recordPublicAgreementWhatsappShare({ agreementId: documentId, token: body.token });
  } else {
    return NextResponse.json({ message: "Unknown document type." }, { status: 404 });
  }
