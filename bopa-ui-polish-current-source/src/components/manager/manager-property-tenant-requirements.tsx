...
type RequirementPreset = Omit<RequirementDraft, "id"> & {
  buttonLabel: string;
};
...
const PRESETS: RequirementPreset[] = [
  {
    ...
    description: "Confirm whether the applicant intends to keep pets.",
    answerType: "yes_no",
  ...
  },
  {
    ...
    description: "Screens applicants who plan to sublet the property.",
    answerType: "yes_no",
    expectedBoolean: false,
    minimumValue: "",
    maximumValue: "",
...
function fromStoredRequirement(
  ...
  return {
    ...
    expectedBoolean: requirement.expected_boolean ?? false,
    minimumValue:
    ...
    requiredGuarantorCount:
      requirement.required_guarantor_count === null
        ? ""
        : String(requirement.required_guarantor_count),
...
function createRequirementDraft(
  ...
  return {
    ...
    requiredGuarantorCount: preset.requiredGuarantorCount,
    mismatchAction: preset.mismatchAction,
    includeInAgreement: preset.includeInAgreement,
    agreementClause: preset.agreementClause,
  };
}

function getAnswerSummary(requirement: RequirementDraft) {
  if (requirement.answerType === "money") {
    return requirement.minimumValue
      ? `At least ₦${Number(
          requirement.minimumValue,
        ).toLocaleString("en-NG")}`
      : "Minimum income not set";
  }

  if (requirement.answerType === "integer") {
    return requirement.maximumValue
      ? `Maximum ${requirement.maximumValue}`
      : "Maximum not set";
  }

  return requirement.expectedBoolean
    ? "Accepted answer: Yes"
    : "Accepted answer: No";
}
...
export function ManagerPropertyTenantRequirements({
  ...
  const [requirements, setRequirements] = useState<
    RequirementDraft[]
  >(initialRequirements.map(fromStoredRequirement));
  const [dismissedToastId, setDismissedToastId] =
    useState<string | null>(null);

  const [state, formAction, isPending] = useActionState(
    saveManagerPropertyTenantRequirementsAction,
    ...
    () =>
      JSON.stringify(
        requirements.map((requirement) => ({
          ...
          agreementClause: requirement.includeInAgreement
  ...
  function addRequirement(preset: RequirementPreset) {
    const alreadyAdded =
      preset.requirementCode !== "custom_yes_no" &&
      requirements.some(
        (requirement) =>
          requirement.requirementCode === preset.requirementCode,
      );
      ...
      >
        ...
        <div className="border-b border-border-soft p-4">
          ...
          <p className="mt-1 text-sm font-semibold leading-6 text-text-muted">
            These requirements will become questions during a new
        ...
        <div className="space-y-5 p-4">
              ...
              {PRESETS.map((preset) => {
                ...
                return (
                  <button
                    ...
                    className="inline-flex min-h-10 items-center rounded-button border border-border-soft bg-white px-3 text-sm font-extrabold text-text-strong transition hover:bg-surface disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    {preset.buttonLabel}
                  </button>
                );
          ...
          <div className="rounded-card bg-warning-soft p-4">
            ...
            <p className="mt-1 text-sm font-semibold leading-6 text-text-muted">
              Do not create requirements based on ethnicity, religion,
              disability, gender, marital status, or other protected personal
              characteristics.
            </p>
          </div>
          ...
          {requirements.length > 0 ? (
                ...
                >
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        When the answer does not qualify
                      </label>

                      <select
                        id={`mismatch-${requirement.id}`}
                        value={requirement.mismatchAction}
                        onChange={(event) =>
                          updateRequirement(
                            requirement.id,
                            (current) => ({
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      value={requirement.questionText}
                      onChange={(event) =>
                        updateRequirement(
                          requirement.id,
                          (current) => ({
                  ...
                  {requirement.answerType === "yes_no" ? (
                    ...
                    <Input
                      ...
                      min="1"
                      step="1"
                      value={requirement.maximumValue}
                      onChange={(event) =>
                        updateRequirement(
                          requirement.id,
                          (current) => ({
                  ...
                  "guarantor_required" ? (
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        value={requirement.requiredGuarantorCount}
                        onChange={(event) =>
                          updateRequirement(
                            requirement.id,
                            (current) => ({
                        ...
                        className="min-h-12 w-full rounded-button border border-border-soft bg-white px-4 text-sm font-semibold text-text-strong outline-none transition focus:border-primary"
                      >
                        <option value="1">One guarantor</option>
                        <option value="2">Two guarantors</option>
                      </select>
                    </div>
                  ) : null}
                  ...
                  <label className="flex gap-3 rounded-card border border-border-soft bg-white p-4 text-sm font-semibold leading-6 text-text-strong">
                    <input
                      type="checkbox"
                      checked={requirement.includeInAgreement}
                      onChange={(event) =>
                        updateRequirement(
                          requirement.id,
                          (current) => ({
                    ...
                    />
                    <span>
                      Reinforce this requirement in the tenancy agreement.
                    </span>
                  </label>
                  ...
                  {requirement.includeInAgreement ? (
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        value={requirement.agreementClause}
                        onChange={(event) =>
                          updateRequirement(
                            requirement.id,
                            (current) => ({
                              ...current,
                              agreementClause: event.target.value,
                            }),
                          )
                        }
                        rows={3}
            ...
            <div className="rounded-card bg-surface p-4">
              ...
              <p className="mt-1 text-sm font-semibold leading-6 text-text-muted">
                New tenant applications will use the standard KYC form only.
              </p>
            </div>
          )}
