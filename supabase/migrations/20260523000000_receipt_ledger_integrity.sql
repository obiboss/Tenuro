...
-- One receipt PDF path per posted payment
...
-- Ledger lookup by payment reference (column name varies by deployment)
do $$
begin
  if exists (
    select 1
    ...
    execute $sql$
      create index if not exists ledger_entries_tenancy_payment_ref_idx
        on public.ledger_entries (tenancy_id, payment_id)
